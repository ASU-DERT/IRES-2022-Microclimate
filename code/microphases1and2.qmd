...
```{r}
#| label: plot-of-time-litter-wet

# (assumes ggpattern, dplyr, tidyr, ggplot2 are loaded and
#  micro1_h_above_15pct_moisture and TYPE_colors exist)
...
# prepare data with explicit ymin/ymax per stack (same as before)
...
# pattern appearance (tweak as desired)
pattern_fill_color <- "white"   # color of hatch lines
pattern_angle <- 45
pattern_density <- 0.6
pattern_spacing <- 0.02

# Use geom_rect_pattern for both layers so 'pattern' appears in legend.
# Bottom: pattern = "none"; Top: pattern = "stripe"
p <- ggplot() +
  # bottom solid segments (0-24 h)
  geom_rect_pattern(
    ...
    pattern = "none",        # explicit (redundant with mapping but clearer)
    colour = NA
  ) +
  # top hatched segments (24-48 h)
  geom_rect_pattern(
    ...
    pattern = "stripe",      # explicit hatched pattern
  ...
  facet_grid(. ~ Rainfall, scales = "free_x", space = "free") +
  scale_fill_manual(values = TYPE_colors_named) +
  # map pattern names to actual pattern types (ensure order matches period factor)
  scale_pattern_manual(values = c("0–24 h" = "none", "24–48 h" = "stripe")) +
  # guides: show fill (TYPE) first, then pattern (Time period)
  guides(
    fill = guide_legend(order = 1, title = "TYPE"),
    pattern = guide_legend(order = 2, title = "Time period",
                           override.aes = list(
                             # make example legend boxes visible regardless of TYPE fills
                             fill = c("grey70", "grey70"),
                             colour = c(NA, NA),
                             pattern_fill = c(pattern_fill_color, pattern_fill_color),
                             pattern_spacing = c(pattern_spacing, pattern_spacing),
                             pattern_density = c(pattern_density, pattern_density),
                             # pattern values in order of factor levels
                             pattern = c("none", "stripe")
                           ))
  ) +
  # x axis labels for TYPE at integer positions
  scale_x_continuous(
