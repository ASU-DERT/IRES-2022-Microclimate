...
micro1_temp_binned_p1 <- Microphase1_iButtons_hourly_df |>
  ...
  ggplot(aes(x = elapsed_bin, y = temp_hourly_mean,
             color = TYPE,
             shape = TYPE,
             linetype = TYPE,
             group = TYPE)) +
...
# save the plot
ggsave(filename = 
       here("output","figures", "microphase1_temperature_by_ppt_binned.jpg"),
       device = "jpeg",
       width = 8, height = 6, dpi = 300)
```
...
```{r}
#| label: compare litter moisture at 32h and 44h

# summarize data to get means and SEs for each Rainfall treatment at 32h and 44h
micro1_32_44_means <- micro1_df |>
  filter(elapsed_time %in% c(32, 44)) |>
  group_by(Rainfall, elapsed_time) |>
  summarise(
    LitterMoisture_mean = mean(LitterMoisturePct, na.rm = TRUE),
    n = n(),
    sd = sd(LitterMoisturePct, na.rm = TRUE),
    se = sd / sqrt(n)
  )

print(micro1_32_44_means)
...
Microphase2_iButtons_hourly_df <- Microphase2_iButtons_df |>
  ...
  filter(elapsed_bin >= 0 & elapsed_bin <= 42) |>
  summarise(
    temp_hourly_mean = mean(temperature, na.rm = TRUE),
    N = n())
#  ) #|>
