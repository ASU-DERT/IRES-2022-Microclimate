...
```{r}
#| label: plot-of-time-litter-wet-clustered

# prepare long data (period is the x axis)
plot_df <- micro1_h_above_15pct_moisture |>
   mutate(
    period = case_when(
      period == "hours_above_15_0_24"  ~ "0–24 h",
  ...
  mutate(
    ...
    # ensure ordering: 0-24 cluster first, then 24-48
    period = factor(period, levels = c("0–24 h", "24–48 h")),
    Rainfall = factor(Rainfall, levels = unique(Rainfall)),
    TYPE = factor(TYPE, levels = unique(TYPE))
  )

# ensure TYPE_colors named
type_levels <- levels(plot_df$TYPE)
if (is.null(names(TYPE_colors)) || !all(type_levels %in% names(TYPE_colors))) {
  TYPE_colors_named <- setNames(TYPE_colors[seq_along(type_levels)], type_levels)
} else {
...
# pattern settings for hatched (0-24 h)
pattern_fill_color <- "white"
pattern_angle <- 45
pattern_density <- 0.6
pattern_spacing <- 0.02

# Plot: x = period, within each period dodge bars by TYPE
p1 <- ggplot(plot_df, aes(x = period, y = hours,
                         fill = TYPE, pattern = period)) +
  geom_col_pattern(
  ...
  scale_fill_manual(values = TYPE_colors_named) +
  # map period -> pattern: hatched for 0-24, solid for 24-48
  scale_pattern_manual(values = c("0–24 h" = "stripe", "24–48 h" = "none")) +
...
# ensure separate, clear legends: TYPE fills then pattern (hatched vs solid)
p <- p + guides(
  fill = guide_legend(order = 1, title = "TYPE"),
...
# save plot
ggsave(filename = file.path(fig_dir, "microphase1_time_wet_grouped.jpg"),
       plot = p1, device = "jpeg", width = 8, height = 6, dpi = 300)
...
p2 <- ggplot(plot_df, aes(x = period, y = pct, fill = TYPE)) +
  geom_col(position = position_dodge(width = 0.9), width = 0.8, colour = NA) +
  facet_grid(. ~ Rainfall,
             labeller = labeller(Rainfall = function(x) paste0(x, " mm Addition")),
             scales = "free_x", space = "free") +
  scale_fill_manual(values = TYPE_colors_named) +
  labs(x = "Time", y = "Time over threshold (%)") +
  scale_y_continuous(limits = c(0, 75), expand = c(0, 0)) +   # change upper limit as desired
  scale_x_discrete(expand = expansion(add = c(0.6, 0.6))) + # widen gap between the two period clusters
  theme_classic(base_size = 16) +
  theme(
    panel.spacing = unit(1, "lines"),
    strip.background = element_rect(fill = "gray80", color = "black"),
