...
p1 <- ggplot(weather_phase2, aes(x=Datetime, colour=Station)) +
  aes(group=Station) +
  geom_bar(aes(y=Fog), stat = "identity") +
  scale_color_manual(values = my_colors) +
  #scale_fill_manual("legend", values = c("Gobabeb" = "#06286b", "Aussinanis" = "#2ba3ff")) +
  geom_line(aes(y=Humidity / coeff), size=1.5) +
  ylim(0,1) +
